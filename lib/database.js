...
class Database {
    constructor(config) {
        ...
        this.config = config; // Store config for potential re-connection logic
    }
    ...
    postData(tableName, data = {}) {
        // Example: Encrypt specific fields if needed, or all.
        // For sensor_data, numerical values are often not encrypted unless required for specific compliance.
        // Let's assume for this example, no fields in sensor_data are encrypted by default.
        // If you wanted to encrypt, you'd do:
        // const dataToInsert = { ...data };
        // if (dataToInsert.some_sensitive_field) {
        //     dataToInsert.some_sensitive_field = this.encrypt(dataToInsert.some_sensitive_field);
        // }
        const dataToInsert = { ...data };
    ...
    _decryptRow(row) {
        ...
        for (const key in decryptedRow) {
            // Apply decryption to fields known to be encrypted.
            // Example: if (key === 'name' || key === 'email') {
            //    decryptedRow[key] = this.decrypt(decryptedRow[key]);
            // }
            // For sensor_data, if no fields were encrypted, this step isn't strictly needed.
            // However, to be generic, we can try to decrypt fields that look encrypted.
            if (typeof decryptedRow[key] === 'string' && decryptedRow[key].includes(':')) {
                ...
                // Attempt to parse numeric values after decryption
